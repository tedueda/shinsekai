...
"""
ブログページデザイン統一スクリプト
greenscreen-youtube-cm-osaka.htmlをテンプレートとして全ブログページを統一
"""
...
# テンプレートHTML（greenscreen-youtube-cm-osaka.htmlベース）
...
def extract_content_from_html(html_content):
    """既存HTMLからコンテンツ部分を抽出"""
    # 簡単なコンテンツ抽出（実際のHTMLに応じて調整が必要）
    content_patterns = [
        r'<div class="blog-post-content"[^>]*>(.*?)</div>',
        r'<main[^>]*>(.*?)</main>',
        r'<article[^>]*>(.*?)</article>',
    ]
    ...
    # フォールバック：基本的なコンテンツを生成
    return f'''
    <p>このブログ記事は現在準備中です。詳細については、スタジオQまでお問い合わせください。</p>
    
    <h2>スタジオQについて</h2>
    <p>大阪・スタジオQは、プロフェッショナルな映像制作に最適な環境を提供しています。完全防音の同録スタジオで、高品質な映像制作をサポートします。</p>
    
    <h2>お問い合わせ</h2>
    <p>撮影のご相談やスタジオ見学をご希望の方は、お気軽にお問い合わせください。</p>
    '''
...
def convert_blog_file(post_data, filename):
    """個別のブログファイルを変換"""
    ...
    try:
        ...
        print(f"ファイル読み込みエラー: {filename}")
        return False
    
    # コンテンツを抽出
    content = extract_content_from_html(original_content)
    ...
    # テンプレートに適用
    new_html = TEMPLATE.format(**template_data)
...
def main():
    ...
    for post in posts:
        ...
        if url.startswith('blog/'):
            ...
            if convert_blog_file(post, filename):
                success_count += 1
    
    print(f"\n📊 変換結果: {success_count}/{total_count} 件成功")
    print("✅ ブログページデザイン統一完了!")
