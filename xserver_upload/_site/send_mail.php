...
$RECAPTCHA_SCORE_THRESHOLD = 0.5; // 許可するスコアの閾値（必要に応じて 0.3～0.7 で調整）
...
function verify_recaptcha($token) {
    ...
    log_debug('reCAPTCHA success=' . (isset($result['success']) && $result['success'] ? 'true' : 'false') . ', score=' . $score);
    
    // スコアが閾値以上で成功とみなす（0.0～1.0、1.0が最も人間らしい）
    return isset($result['success']) && $result['success'] && 
           isset($result['score']) && $result['score'] >= $RECAPTCHA_SCORE_THRESHOLD;
}
